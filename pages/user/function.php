...
if(isset($_POST['btn_add'])){
    ...
    $su = mysqli_query($con,"SELECT * from tblteacher where username = '".$txt_uname."' ");
    $ct = mysqli_num_rows($su);
    
    if($ct == 0){
        $query = mysqli_query($con,"INSERT INTO tblteacher (name,username,password) 
...
if(isset($_POST['btn_save']))
{
    ...
    $su = mysqli_query($con,"SELECT * from tblteacher where username = '".$txt_edit_uname."' ");
    $ct = mysqli_num_rows($su);
    
    if($ct == 0){
        $update_query = mysqli_query($con,"UPDATE tblteacher set name = '".$txt_edit_name."', username = '".$txt_edit_uname."', password = '".$txt_edit_pass."' where id = '".$txt_id."' ") or die('Error: ' . mysqli_error($con));
