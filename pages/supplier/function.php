...
if (isset($_POST['btn_add'])) {
    ...
    $products_ordered = mysqli_real_escape_string($con, $_POST['txt_products_ordered']);

    $query = mysqli_query($con, "INSERT INTO tblsupplier (name, contact_number, products_ordered) 
                                 VALUES ('$name', '$contact_number', '$products_ordered')")
                                 or die('Error: ' . mysqli_error($con));

    if ($query) {
        if (isset($_SESSION['role'])) {
            $action = 'Added Supplier with Name: ' . $name . ', Contact Number: ' . $contact_number . ', Products Ordered: ' . $products_ordered;
...
if (isset($_POST['btn_save'])) {
    ...
    if ($update_query) {
        if (isset($_SESSION['role'])) {
            $action = 'Edited Supplier with ID: ' . $id . ', Name: ' . $name . ', Contact Number: ' . $contact_number . ', Products Ordered: ' . $products_ordered;
            $iquery = mysqli_query($con, "INSERT INTO tbllogs (user, logdate, action) 
                                          VALUES ('" . $_SESSION['userid'] . "', NOW(), '" . $action . "')");
        }
        $_SESSION['edited'] = 1;
